  ...
  for (const [name, source] of Object.entries(sources)) {
    ...
    try {
      ...
      const input = isObject(source.input)
        ...
        : {};
      data[name] = await service.call(source.service, input);
    } catch (error) {
      data[name] = null;
      errors.push(
...
async function applyEffects(
  ...
  for (const rawEffect of effects) {
    ...
    switch (rawEffect.effect) {
      ...
      case "navigate":
      case "replace": {
        const target = text(rawEffect.to);
        const params = isObject(rawEffect.params)
          ? (bindJson(rawEffect.params, scope) as JsonObject)
          : {};
        session.params.set(target, params);
        session.currentScreen = target;
        if (rawEffect.effect === "replace") {
          session.history[session.history.length - 1] = target;
          await session.nav.replace(target);
        } else {
          session.history.push(target);
          await session.nav.rebuild(target);
        }
        break;
      }
      case "back":
        if (session.history.length > 1) session.history.pop();
        session.currentScreen =
          session.history[session.history.length - 1] ?? session.currentScreen;
        session.nav.back();
        await session.nav.rebuild();
        break;
      case "refresh":
...
async function runAction(
  ...
  await session.nav.runTask(status, async () => {
    ...
    const extraInput = isObject(triggerInput)
      ...
      : {};
    try {
      const result = await service.call(call.service as string, {
        ...baseInput,
        ...extraInput,
      });
...
async function renderScreen(
  ...
  if (session.currentScreen !== text(screen.id)) return;
...
export function registerDeclarativeFeature(input: JsonObject): {
  ok: boolean;
  error?: string;
} {
  const feature = input.feature;
  const files = input.screens;
  if (
    !isObject(feature) ||
    feature.formatVersion !== 1 ||
    typeof feature.moduleId !== "string"
  ) {
    return { ok: false, error: "feature 必须是 formatVersion=1 的对象" };
  }
  if (
    !isObject(files) ||
    !Array.isArray(feature.screens) ||
    !Array.isArray(feature.entries)
  ) {
    return { ok: false, error: "screens 或 entries 格式错误" };
  }
  unregisterDeclarativeFeature(feature.moduleId);
  const screens = new Map<string, JsonObject>();
  for (const reference of feature.screens) {
    if (
      !isObject(reference) ||
      typeof reference.id !== "string" ||
      typeof reference.file !== "string"
    ) {
      return { ok: false, error: "页面引用格式错误" };
    }
    const screen = files[reference.file];
    if (
      !isObject(screen) ||
      screen.id !== reference.id ||
      !Array.isArray(screen.body)
    ) {
      return { ok: false, error: `页面 ${reference.id} 缺失或 id 不一致` };
    }
    screens.set(reference.id, screen);
  }
...
/** 注销 feature 及其派生入口。 */
export function unregisterDeclarativeFeature(moduleId: string): {
  ok: boolean;
} {
